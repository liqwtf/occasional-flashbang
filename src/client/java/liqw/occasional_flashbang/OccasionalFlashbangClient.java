...
public class OccasionalFlashbangClient implements ClientModInitializer {
    ...
    private static final Identifier FLASHBANG_IDENTIFIER = Identifier.fromNamespaceAndPath(OccasionalFlashbang.MOD_ID, "flashbang");
    public static final SoundEvent FLASHBANG_SOUND = SoundEvent.createVariableRangeEvent(FLASHBANG_IDENTIFIER);
    private static final Identifier LOBOTOMY_IDENTIFIER = Identifier.fromNamespaceAndPath(OccasionalFlashbang.MOD_ID, "lobotomy");
    public static final SoundEvent LOBOTOMY_SOUND = SoundEvent.createVariableRangeEvent(LOBOTOMY_IDENTIFIER);
    ...
    public void onInitializeClient() {
        ...
        Registry.register(BuiltInRegistries.SOUND_EVENT, FLASHBANG_IDENTIFIER, FLASHBANG_SOUND);
        Registry.register(BuiltInRegistries.SOUND_EVENT, LOBOTOMY_IDENTIFIER, LOBOTOMY_SOUND);

        FlashbangConfig config = AutoConfig.getConfigHolder(FlashbangConfig.class).getConfig();

        ClientTickEvents.END_CLIENT_TICK.register(client -> {
            if (client.player == null || client.isPaused() || !config.enabled) return;
        ...
        HudElementRegistry.attachElementBefore(VanillaHudElements.HOTBAR, FLASHBANG_IDENTIFIER, OccasionalFlashbangClient::render);
    }
}